...
export default function VisitRoom() {
  ...
  useEffect(() => {
    ...
    // The doctor's "waiting for patient" screen was previously stuck forever:
    // room data was only fetched once on mount, and VideoCall (the only thing
    // that listened for the patient joining) never even rendered while
    // waitingForPatient was true — a deadlock. Refresh room state here as
    // soon as anyone joins, so the page flips out of the waiting screen live.
    socket.on("peer:joined", () => {
      loadRoom();
    });
    // Safety net: if the doctor completes the visit while the patient hasn't
    // opened the call yet (e.g. still on the waiting screen), still redirect them.
  ...
  // Passed to VideoCall so there's always an explicit way out of a visit,
  // for both roles, regardless of whether the peer is still connected.
        ...
        >
          <div>
            <div className="flex items-center gap-12">
              ...
              <span
                className={`badge ${room.status === "open" ? "badge-open" : room.status === "active" ? "badge-active" : "badge-completed"}`}
              >
            ...
            <p className="text-muted text-sm mt-8">
              ...
              {isDoctor
                ? ` · Patient: ${room.patient_name || "Not joined yet"}`
        ...
        {waitingForPatient ? (
          <div className="card" style={{ padding: 40, textAlign: "center" }}>
            ...
            <p className="text-muted" style={{ marginBottom: 20 }}>
              Share this code so they can join the visit:
            </p>
            <div
              style={{
                fontSize: 36,
...
// Wider ratio than before (was 1.4fr / 1fr) so the call is the clear focal
// point of the page instead of competing evenly with the sidebar.
const roomGrid = {
  display: "grid",
