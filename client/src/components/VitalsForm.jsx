...
export default function VitalsForm({ roomId, patientId, onSaved }) {
  const { token } = useAuth();
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setSavedMsg('Saved to visit record ✓');
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="field">
        <label>Notes</label>
        <textarea rows={2} value={form.notes} onChange={e => update('notes', e.target.value)} placeholder="Any observations from the visit…" />
      </div>

      <button className="btn btn-primary btn-block" disabled={saving}>
        {saving ? 'Saving…' : 'Save vitals to visit record'}
      </button>
