...
app.post('/api/auth/signup', (req, res) => {
  const { name, email, password, role, age, gender, phone, medical_conditions, specialization } = req.body;
  ...
  db.prepare(`
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?)
  `).run(id, name, email, password_hash, role, age || null, gender || null, phone || null, medical_conditions || null, specialization || null);
...
app.post('/api/auth/login', (req, res) => {
  ...
  if (role && user.role !== role) return res.status(401).json({ error: `No ${role} account found for this email` });
...
app.post('/api/rooms', auth('doctor'), (req, res) => {
  ...
  while (db.prepare('SELECT id FROM rooms WHERE code = ?').get(code)) code = genRoomCode();

  db.prepare(`INSERT INTO rooms (id, code, title, doctor_id, status) VALUES (?,?,?,?, 'open')`)
...
app.post('/api/rooms/join', auth('patient'), (req, res) => {
  ...
  db.prepare(`UPDATE rooms SET patient_id = ?, status = 'active' WHERE id = ?`).run(req.user.id, room.id);
...
app.get('/api/rooms/:code', auth(), (req, res) => {
  ...
  if (req.user.role === 'patient' && room.patient_id && room.patient_id !== req.user.id) {
    return res.status(403).json({ error: 'Not your room' });
  }
  res.json({ room });
});
...
app.post('/api/rooms/:code/complete', auth('doctor'), (req, res) => {
  ...
  if (!room || room.doctor_id !== req.user.id) return res.status(404).json({ error: 'Room not found' });
  db.prepare(`UPDATE rooms SET status='completed', completed_at = CURRENT_TIMESTAMP WHERE id = ?`).run(room.id);
  res.json({ ok: true });
});

// ---------- VITALS ROUTES ----------
app.post('/api/vitals', auth('doctor'), (req, res) => {
  const { room_id, patient_id, stage, temperature, bp_systolic, bp_diastolic, sugar, spo2, heart_rate, dosage_given, notes } = req.body;
  if (!room_id || !patient_id || !stage) return res.status(400).json({ error: 'Missing required fields' });
  const id = uuidv4();
  db.prepare(`
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
  `).run(id, room_id, patient_id, req.user.id, stage, temperature || null, bp_systolic || null, bp_diastolic || null, sugar || null, spo2 || null, heart_rate || null, dosage_given || null, notes || null);
...
app.get('/api/vitals/room/:roomId', auth(), (req, res) => {
  const rows = db.prepare('SELECT * FROM vitals WHERE room_id = ? ORDER BY recorded_at ASC').all(req.params.roomId);
  res.json({ vitals: rows });
});

app.get('/api/vitals/patient/:patientId', auth(), (req, res) => {
  const rows = db.prepare('SELECT * FROM vitals WHERE patient_id = ? ORDER BY recorded_at ASC').all(req.params.patientId);
  res.json({ vitals: rows });
});

// ---------- DOCTOR DASHBOARD ANALYTICS ----------
app.get('/api/dashboard/doctor', auth('doctor'), (req, res) => {
  const rooms = db.prepare('SELECT * FROM rooms WHERE doctor_id = ?').all(req.user.id);
  const roomIds = rooms.map(r => r.id);

  let vitals = [];
  if (roomIds.length) {
    const placeholders = roomIds.map(() => '?').join(',');
    vitals = db.prepare(`SELECT * FROM vitals WHERE room_id IN (${placeholders}) ORDER BY recorded_at ASC`).all(...roomIds);
  }

  const totalPatients = new Set(rooms.filter(r => r.patient_id).map(r => r.patient_id)).size;
  ...
  const nums = (field) => vitals.map(v => v[field]).filter(v => v !== null && v !== undefined);
  ...
  res.json({ totalPatients, totalVisits, activeVisits, completedVisits, summary, trend, statusBreakdown, rooms, vitals });
});

app.get('/api/dashboard/patient', auth('patient'), (req, res) => {
  const rooms = db.prepare('SELECT * FROM rooms WHERE patient_id = ?').all(req.user.id);
  const vitals = db.prepare('SELECT * FROM vitals WHERE patient_id = ? ORDER BY recorded_at ASC').all(req.user.id);
...
io.on('connection', (socket) => {
  socket.on('room:join', ({ roomCode, userId, userName, role }) => {
    ...
    socket.to(roomKey).emit('peer:joined', { userId, userName, role, socketId: socket.id });
  });

  socket.on('webrtc:offer', ({ roomCode, offer, to }) => {
    io.to(to).emit('webrtc:offer', { offer, from: socket.id, userName: socket.data?.userName });
  });
  ...
  socket.on('disconnect', () => {
    if (socket.data?.roomCode) {
      socket.to(`room:${socket.data.roomCode}`).emit('peer:left', { socketId: socket.id });
    }
  });
});
